module.exports = (link, options, cookie, script, headlessType) => {
  ...
  jsonLoad.create = async () => {
    ...
    try {
      script = fs.readFileSync("./injection.js", "utf8");
    } catch (err) {
      console.error("Error reading local script:", err);
      throw err; // Re-throw the error to indicate failure
    }
    // if ((script ?? "") === "") {
    //   // Fetch the tiktok-to-ytdlp script
    //   let req = await fetch(
    //     "https://raw.githubusercontent.com/Dinoosauro/tiktok-to-ytdlp/main/script.js"
    //   );
    //     script = await req.text();
    // }
    if ((options ?? "") !== "") {
      // If the user has provided options
      let extraOptions = []; // A dobule array, that'll contain [["the property name", "and its values"]]
      for (let option in options) {
        // For each item in the object
        if (typeof options[option] === "object") {
          // If it's another object, do this loop again
          for (let suboption in options[option]) {
            extraOptions.push([
              `${option}=>${suboption}`,
              options[option][suboption],
            ]); // Write also the main object
          }
        } else extraOptions.push([option, options[option]]); // Not an object: automatically change property
      }
      script = script.replace(
        "nodeElaborateCustomArgs()",
        `nodeElaborateCustomArgs(JSON.parse('${JSON.stringify(extraOptions)}'))`
      ); // Adapt the script to change the custom options.
    }
    ...
    let browser = await puppeteer.launch({ headless: false ?? "new" }); // Create a new Puppeteer
    ...
    // await puppeteer.screenshot({path: 'ex.png', fullPage: true})
      ...
      if (isLoaded)
        isLoaded =
          (await page
            ...
            .toString()
            .replaceAll(" ", "")
            .indexOf("<body>Pleasewait")) === -1; // Look if the user must wait
      if (isLoaded)
        isLoaded = await page.evaluate(
          `document.querySelectorAll(".tiktok-x6y88p-DivItemContainerV2, .css-x6y88p-DivItemContainerV2").length !== 0`
        ); // Look if there are any DivContainers
      if (!isLoaded) {
        // If one of the checks above failed, wait another 5 seconds.
        function wait() {
    ...
    await checkIfUrl();
    return {
      // Return a new object that'll permit to start TikTok loading.
      start: async () => {
