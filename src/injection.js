...
var scriptOptions = {
    scrolling_min_time: 3000, // Change the mininum time the script will try to refresh the page
    scrolling_max_time: 4000, // Change the maxinum time the script will try to refresh the page
...
function loadWebpage() {
    if (document.querySelectorAll(".tiktok-qmnyxf-SvgContainer").length === 0) {
        ...
        setTimeout(() => {
            if (height !== document.body.scrollHeight) {
                ...
                setTimeout(() => {
                    if (
                        document.querySelectorAll(".tiktok-qmnyxf-SvgContainer").length ===
                        0 &&
                        height == document.body.scrollHeight
                    ) {
...
function addArray() {
    var getClass = document.querySelectorAll(
        ".tiktok-x6y88p-DivItemContainerV2, .css-x6y88p-DivItemContainerV2"
    ...
    for (var i = 0; i < getClass.length; i++) {
        ...
        var getLink = scriptOptions.advanced.get_link_by_filter
            ? Array.from(getClass[i].querySelectorAll("a")).filter(
                (e) => e.href.indexOf("/video/") !== -1
            )[0]?.href
            : (
                getClass[i].querySelector("[data-e2e=user-post-item-desc]") ??
                getClass[i].querySelector("[data-e2e=user-liked-item]") ??
                getClass[i].querySelector("[data-e2e=music-item]") ??
                getClass[i].querySelector("[data-e2e=user-post-item]") ??
                getClass[i].querySelector("[data-e2e=favorites-item]") ??
                getClass[i].querySelector("[data-e2e=challenge-item]")
        ...
        if (scriptOptions.advanced.check_nullish_link && (getLink ?? "") === "") {
        ...
        if (
            containerSets[0].indexOf(getLink) === -1 &&
            skipLinks.indexOf(getLink) === -1
        ) {
            // If the link hasn't been used, add it to the ContainerSets.
            containerSets[0].push(getLink);
            containerSets[1].push(
                (getClass[i].querySelector("[data-e2e=video-views]")?.innerHTML ?? "0")
                    .replace("K", "00")
                    .replace("M", "00000")
            );
        }
...
function downloadScript(script) {
    ...
    if (scriptOptions.node.isNode) {
        if (scriptOptions.node.isResolveTime)
            scriptOptions.node.resolve(script.split("\n"));
    ...
    scriptOptions.output_name_type // Look at the type of the name
        ...
        case 2: // Fetch name from the first "h1" element on the page
            name = `${document.querySelector("h1")?.textContent.trim() ?? "TikTokLinks"
                }.txt`;
            break;
    }
